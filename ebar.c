...
static void apply_css(void) {
    ...
    const char *css =
        "* { font-family: JetBrainsMonoNerdFont; background: none; box-shadow: "
        "none; border: none; } "
        "window, .background { "
    ...
    if (fp) {
        ...
        while ((p = strstr(p, "{"))) {
            ...
            if (addr_p && ws_p && (!next || (addr_p < next && ws_p < next))) {
                ...
                // -> 55c7...)
                char *addr_to_map = (strncmp(raw_addr, "0x", 2) == 0)
                                        ? g_strdup(raw_addr + 2)
                                        : g_strdup(raw_addr);
                ...
                if (id_p) {
                    ...
                    if (id >= 1 && id <= MAX_WORKSPACES) {
                        w->ws_win_count[id]++;
                        g_hash_table_insert(w->window_map, addr_to_map,
                                            GINT_TO_POINTER(id));
                    } else {
                        g_free(addr_to_map);
                    }
    ...
    for (GList *l = w->bar_windows; l != NULL; l = l->next) {
        ...
        for (int i = 0; i < MAX_WORKSPACES; i++) {
            ...
            GtkStyleContext *context =
                gtk_widget_get_style_context(bw->ws_labels[i]);
...
static void handle_ipc_line(AppWidgets *w, char *line) {
    if (strncmp(line, "workspace>>", 11) == 0) {
        ...
        g_hash_table_insert(w->window_map, g_strdup(addr),
                            GINT_TO_POINTER(new_ws));
...
static GtkWidget *create_metric_box(const char *icon_str,
    ...
    if (METRIC_USE_BARS) {
        *out_widget =
        ...
        gtk_style_context_add_class(gtk_widget_get_style_context(*out_widget),
                                    "metric-scale");
    } else {
...
static void create_bar_window(GdkMonitor *monitor, AppWidgets *widgets) {
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(metrics_grid),
                                "metrics");
