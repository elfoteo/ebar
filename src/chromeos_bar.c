...
#define A(...) n += snprintf(css+n, (int)sizeof(css)-n, __VA_ARGS__)
    ...
    A(".cb-pill { background-color: #505153; color: #E8EAED; border-radius: 18px; "
      "  padding: 0 16px; font-size: 13px; font-weight: 500; min-height: 36px; } ");
    ...
    A(".cb-desk-arrow { background: transparent; color: #E8EAED; border-radius: 6px; "
      "  min-width: 18px; min-height: 28px; padding: 0 2px; font-size: 14px; "
    ...
    for (GList *l = state->bar_windows; l != NULL; l = l->next) {
        ...
        if (bw->monitor) {
            ...
            const char *br = bw->has_fullscreen ? "0" : "24px 24px 0 0";
            A("#chromebook-bar-%d-%d { background-color: #424348; border-radius: %s; padding: 6px; } ",
              mgeom.x, mgeom.y, br);
        }
    }
